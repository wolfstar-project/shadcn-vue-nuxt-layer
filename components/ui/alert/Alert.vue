...
<template>
	<Primitive :as="as" :data-orientation="orientation" :class="ui.root({ class: [props.class, props.ui?.root] })">
		...
		<div :class="ui.wrapper({ class: props.ui?.wrapper })">
			<div v-if="title || !!slots.title" :class="ui.title({ class: cn('mb-1 font-medium leading-none tracking-tight', props.ui?.title) })">
				<slot name="title">
					{{ title }}
				</slot>
			</div>
...
import { alert, type AlertEmits, type AlertProps, type AlertSlots } from '.';
